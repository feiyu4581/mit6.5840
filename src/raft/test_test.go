...
func TestReElection2A(t *testing.T) {
	...
	fmt.Printf("1 Test (2A): election after network failure: disconnected leader: %d\n", leader1)
	cfg.checkOneLeader()
	...
	cfg.connect(leader1)
	fmt.Printf("2 Test (2A): election after network failure: reconnected old leader: %d\n", leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	cfg.disconnect(leader2)
	fmt.Printf("3 Test (2A): election after network failure: disconnected leader: %d\n", leader2)
	cfg.disconnect((leader2 + 1) % servers)
	fmt.Printf("4 Test (2A): election after network failure: disconnected next: %d\n", (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	...
	fmt.Printf("5 Test (2A): election after network failure: reconnected next: %d\n", (leader2+1)%servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	fmt.Printf("6 Test (2A): election after network failure: reconnected leader: %d\n", leader2)
	cfg.checkOneLeader()
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		fmt.Printf("[%d]Test (2A): current=%d, multiple elections: disconnected %d %d %d\n", ii, current, i1, i2, i3)
		cfg.disconnect(i1)
		cfg.disconnect(i2)
		cfg.disconnect(i3)
...
func TestFigure82C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		if leader != -1 {
			fmt.Printf("Test (2C): Figure 8: leader %d crashes\n", leader)
			cfg.crash1(leader)
			nup -= 1
		}
		...
		if nup < 3 {
			...
			if cfg.rafts[s] == nil {
				...
				cfg.connect(s)
				fmt.Printf("Test (2C): Figure 8: restart server %d\n", s)
				nup += 1
			}
		}
	}
